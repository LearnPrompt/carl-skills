...
随口跟 Agent 说一句「收拾一下 Downloads」或者「磁盘满了」，它先只读扫一遍，然后在浏览器里打开一份报告。整理页上有目录杂乱度、每个文件的去向、成对的压缩包和重复副本。盘点页上有磁盘总览、最大的五项、清理优先级。每一项都写着这是什么、为什么这么判、动了会怎样。你在页面上勾选、单选、点按钮，每次都有二次确认，动过的能一键撤回。
